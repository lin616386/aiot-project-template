# 个性化
- 样式前缀自定义
    - common/components/i18nProvider prefixCls='自定义类'
    - common/styles/theme.less @ant-prefix @template-prefix变量修改样式前缀配置-自定义类
- antd: 4.13.1
- ucf-config/config
    - 删除不必要的配置文件变量，并自行在入口index.html引入
    - 如果window.appConfig改为其他名字，在工具ucf-common/utils/app.js中涉及拦截登陆等引用处修改为统一配置名称,全局替换
    - 是否嵌入乾坤微前端的判断，ucf-common/utils/tools.js 判断来源配置文件的变量如有需要，可修改
- 公共工具方法切换至组件库（组件库已经封装的方法）
- package.json依赖安装包只留下依赖的包
- 

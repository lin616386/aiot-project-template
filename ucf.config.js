...
const context = 'iot-mgr-platform-fe';
	...
	return {
		...
		global_env: {
			...
			'process.env.STATIC_HTTP_PATH':
				env === 'development' ? JSON.stringify('/static') : JSON.stringify('../static'),
			'process.env.STATIC_HTTP_PATH':
				env === 'development'
					? JSON.stringify(`${context ? `/${context}` : ''}/static`)
					: JSON.stringify('../static'),
			'process.env.API_BASE_URL': env === 'development' ? JSON.stringify('') : JSON.stringify('/iot-mgr-platform-fe'),
		},
